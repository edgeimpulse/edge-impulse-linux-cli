...
import { DataForwarder } from "../library/data-forwarder";
...
describe("endpoint config", () => {
    ...
    it("uses EI_CLI_INGESTION_ENDPOINT in DataForwarder", () => {
        process.env.EI_CLI_INGESTION_ENDPOINT = "http://host.docker.internal:14810";

        const dataForwarder = new DataForwarder({
        ...
        assert.strictEqual(
            (<any>dataForwarder)._options.ingestionHost,
            "http://host.docker.internal:14810"
        );
    });

    it("keeps explicit DataForwarder host behavior", () => {
        process.env.EI_CLI_INGESTION_ENDPOINT = "http://host.docker.internal:14810";
        ...
        assert.strictEqual((<any>dataForwarder)._options.ingestionHost, "http://localhost:4810");
    });
});
